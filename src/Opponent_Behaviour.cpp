...
typedef enum { REAR_LEFT = 0, REAR_RIGHT, FRONT, NUM_OPP_WHEEL_POSITIONS } OppWheelPositionType;
...
static const unsigned char kClassicStandardOpponentSpeedValues[NUM_TRACKS][MAX_PIECES_PER_TRACK] = {
    {/* Little Ramp data */
     ...
     0x64, 0x5a, 0x50, 0x46, 0x7a, 0x70, 0x66, 0x5c, 0x52, 0x48, 0x48, 0x48, 0x48, 0x7c},
    {/* Stepping Stones data */
     0xf2, 0xe8, 0xde, 0xd4, 0x67, 0x5d, 0x53, 0x49, 0x3f, 0x4b, 0x41, 0x41, 0xc1, 0xd2, 0xc8, 0xbe, 0xc7, 0xbd, 0xc5,
     0xbb, 0xc4, 0xba, 0x55, 0x4b, 0x41, 0x41, 0x41, 0x60, 0x56, 0x4c, 0x42, 0x7d, 0x7d, 0x73, 0x69, 0x5f, 0x55, 0x4b,
     0x41, 0x41, 0x41, 0xfd, 0xfd, 0xfd, 0xf3, 0x7d, 0x7d, 0x73, 0x69, 0x5f, 0x55, 0x4b, 0x41, 0x41, 0x41, 0x7c},
    {/* Hump Back data */
     0x52, 0x4d, 0x77, 0x77, 0x77, 0x6d, 0x63, 0x59, 0x4f, 0x45, 0x45, 0x45, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77,
     0x77, 0x6d, 0x63, 0x59, 0x4f, 0x45, 0x45, 0x45, 0x56, 0x4c, 0x77, 0x77, 0x6d, 0x63, 0x59, 0x4f, 0x45, 0x45,
     0x45, 0x4f, 0x61, 0x57, 0x4d, 0x45, 0x4f, 0x45, 0x45, 0x63, 0x59, 0x4f, 0x45, 0x45, 0x45, 0x66, 0x5c},
    {/* Big Ramp data */
     0x7a, 0x7a, 0x7a, 0x7a, 0x7a, 0x7a, 0x70, 0x66, 0x5c, 0x52, 0x48, 0x48, 0x48, 0x58, 0x4e,
     0x4b, 0x69, 0x5f, 0x55, 0x4b, 0x46, 0x66, 0x5c, 0x52, 0x48, 0x48, 0x48, 0x48, 0x7e, 0xf4,
     0xea, 0xe0, 0xd6, 0x7a, 0x7a, 0x70, 0x66, 0x5c, 0x52, 0x48, 0x48, 0x48, 0x48, 0x7c},
    {/* Ski Jump data */
    ...
    {/* High Jump data */
     0xe7, 0xdd, 0xd3, 0x77, 0x77, 0x77, 0x77, 0x6d, 0x63, 0x59, 0x4f, 0x4f, 0x4f, 0x7a, 0x7a, 0x7a, 0x7a, 0x7a, 0x70,
     0x66, 0x5c, 0x52, 0x52, 0x55, 0x59, 0x4f, 0x4f, 0x77, 0x77, 0x77, 0x77, 0x6d, 0x63, 0x59, 0x4f, 0x4f, 0xcf, 0xe7,
     0xdd, 0xd3, 0xce, 0x77, 0x77, 0x77, 0x77, 0x6d, 0x63, 0x59, 0x4f, 0x4f, 0x4f, 0x7c, 0x41, 0x41, 0x41, 0x7c},
    {/* Roller Coaster data */
...
WCHAR* opponentNames[NUM_OPPONENTS] = {L"Hot Rod     ", L"Whizz Kid   ", L"Bad Guy     ", L"The Dodger  ",
                                       ...
                                       L"Bully Boy   ", L"Jumping Jack", L"Road Hog    "};
...
static unsigned char opp_track_speed_values[] = //DAT.1fe2c
    {
        // Standard league
        0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x41, 0x3a, 0x3e, 0x41, 0x48, 0x51, 0x48, 0x4f, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00,             // used when creating opponents.speed.values
        0x48, 0x41, 0x45, 0x48, 0x4f, 0x58, 0x4f, 0x56, // used when creating opponents.speed.values

        // Super league
        0x07, 0x03, 0x03, 0x03, 0x03, 0x03, 0x07, 0x03, 0x66, 0x57, 0x57, 0x59, 0x59, 0x69, 0x62, 0x64, 0x07, 0x03,
        0x03, 0x03, 0x03, 0x01, 0x03, 0x03,            // used when creating opponents.speed.values
...
static void CalculateWheelDifference(long road_height, long actual_height, long height_adjust,
                                     long* old_difference_in_out, long* new_difference_out, long* touching_road);
...
void OpponentBehaviour(long* x, long* y, long* z, float* x_angle, float* y_angle, float* z_angle,
     ...
     */
    opponent_x = (opp_front_left_road_pos.x + opp_front_right_road_pos.x + opp_rear_left_road_pos.x +
                  opp_rear_right_road_pos.x) /
                 4;
    ...
    vis_rear_left_y = opp_rear_left_road_pos.y > opp_actual_height[REAR_LEFT] ? opp_rear_left_road_pos.y
                                                                              : opp_actual_height[REAR_LEFT];
    vis_rear_right_y = opp_rear_right_road_pos.y > opp_actual_height[REAR_RIGHT] ? opp_rear_right_road_pos.y
     ...
     */
    opponent_z = (opp_front_left_road_pos.z + opp_front_right_road_pos.z + opp_rear_left_road_pos.z +
                  opp_rear_right_road_pos.z) /
                 4;
...
#ifdef USE_ROAD_Y
    double yd = static_cast<double>(rear_y - opp_front_road_pos_y) /
                2; // Note y is halved because of unit differences between y and x,z
#else
#ifdef NEW_OPP_METHOD
    double yd = static_cast<double>(rear_y - vis_front_y) /
                2; // Note y is halved because of unit differences between y and x,zn y and x,z
#else
    double yd = static_cast<double>(rear_y - opp_actual_height[FRONT]) /
                2; // Note y is halved because of unit differences between y and x,z
#endif
#endif
...
#ifdef USE_ROAD_Y
    yd = static_cast<double>(opp_rear_left_road_pos.y - opp_rear_right_road_pos.y) /
         2; // Note y is halved because of unit differences between y and x,z
#else
#ifdef NEW_OPP_METHOD
    yd = static_cast<double>(vis_rear_left_y - vis_rear_right_y) /
         2; // Note y is halved because of unit differences between y and x,zn y and x,z
#else
    yd = static_cast<double>(opp_actual_height[REAR_LEFT] - opp_actual_height[REAR_RIGHT]) /
         2; // Note y is halved because of unit differences between y and x,z
#endif
#endif
    ...
    */
    ...
    if (GetRecordedAmigaWord(&temp)) {
        ...
        if (flag != opponents_required_z_speed_reached) {
            ...
            fprintf(out, "%s different %d %d (VALUE2 %d)\n", "opponents.required.z.speed.reached", temp,
                    opponents_required_z_speed_reached, VALUE2);
    ...
    if (opponents_distance_into_section >= (Track[opponents_current_piece].numSegments * 256)) {
        ...
        if (opponents_current_piece > (NumTrackPieces - 1))
            opponents_current_piece = 0;
    }
...
static void UpdateOpponentsActualWheelHeights(void) {
    ...
    CalculateWheelDifference(opp_rear_left_road_pos.y, opp_actual_height[REAR_LEFT], height_adjust,
                             &opp_old_rear_left_difference, &opp_new_rear_left_difference, &touching_road);

    CalculateWheelDifference(opp_rear_right_road_pos.y, opp_actual_height[REAR_RIGHT], height_adjust,
                             &opp_old_rear_right_difference, &opp_new_rear_right_difference, &touching_road);

    CalculateWheelDifference(opp_front_road_pos_y, opp_actual_height[FRONT], height_adjust, &opp_old_front_difference,
                             &opp_new_front_difference, &touching_road);
    ...
    long temp;
    if (GetRecordedAmigaWord(&temp)) {
        if ((temp && !opp_touching_road) || (!temp && opp_touching_road)) {
            ++VALUE1; // Count differences
    ...
    opp_y_acceleration[REAR_LEFT] =
        (total_diff + opp_new_rear_left_difference + (opp_new_rear_left_difference << 2)) >> 3;
    opp_y_acceleration[REAR_RIGHT] =
...
static void CalculateWheelDifference(long road_height, long actual_height, long height_adjust,
    ...
    if (new_difference < 0) {
        // wheel above road
        if (new_difference < -96)
            new_difference = -96; // set to maximum amount above road
    }
    ...
    if (amount_below_road < 0)
        amount_below_road = 0;
    if (amount_below_road > 1023)
        amount_below_road = 1023;
...
*/
    ...
    long B63ce1 =
        static_cast<long>(rand()) & static_cast<long>(opp_track_speed_values[track_id + 16 + (bSuperLeague ? 32 : 0)]);
...
static long TAB5be34[] = {0x20,  0x50,  0x60,  0x70,  0x70,  0x60,  0x50,  0x20,
...
ros2:
    ...
    if (B1bbbd & 0x80)
        B1bbbd = B1bbbd - 0x100; //sign extend again
#endif
...
static void OpponentPlayerInteraction(void) {
    ...
    if (B1bbbd & 0x80)
        B1bbbd = B1bbbd - 0x100; //sign extend
