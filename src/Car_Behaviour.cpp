...
long front_left_damage = 0, front_right_damage = 0, rear_damage = 0;
long damaged = 0;
long new_damage = 0;
long nholes = 0;
...
/*    Static data */
/*    =========== */
static long player_x, player_z;

static long player_x_angle = 0, player_y_angle = 0, player_z_angle = 0;

static long player_world_x_speed = 0, player_world_y_speed = 0, player_world_z_speed = 0;

static long player_x_speed = 0, player_y_speed = 0;
...
static long gravity_x_acceleration, gravity_y_acceleration, gravity_z_acceleration;
...
static long old_front_left_difference = 0, old_front_right_difference = 0, old_rear_difference = 0;

static long smashed_countdown = 0;

static long car_to_road_collision_z_acceleration;

static long player_x_acceleration, player_y_acceleration, player_z_acceleration;
...
static long player_x_rotation_acceleration, player_y_rotation_acceleration, player_z_rotation_acceleration;
...
static long CalcDistanceOffRoad(long x, long z, long ox, long oz, long ux, long uz, long vx, long vz, long* ex,
                                long* ez);
static void CalcSurfacePosition(long piece, long x, long z, long ox, long oz, long ux, long uz, long vx, long vz,
                                long* sx, long* sz, long* road_x, long* segment_out);
...
static long CalcSectionYAngle(long piece, long x, long z);
...
static void CarControl(DWORD input) {
    ...
    long left = (input & KEY_P1_LEFT), right = (input & KEY_P1_RIGHT), boost = (input & KEY_P1_BOOST);
    ...
    BoostPower(boost_flag, accelerate, brake);
...
static void BoostPower(long boost_flag, long accel_flag, long brake_flag) {
    boost_activated = 0;
...
static void CarMovement(void) {
    ...
    if (StartOfAmigaRecording) {
        GetRecordedAmigaLong(&player_x);
        player_x *= (PC_FACTOR * 4);
        GetRecordedAmigaLong(&player_y);
        GetRecordedAmigaLong(&player_z);
        player_z *= (PC_FACTOR * 4);
...
static long GetPieceUsingMap(long x, long z, long* piece_out) {
    ...
    if (((map_x < 0) || (map_x >= NUM_TRACK_CUBES)) || ((map_z < 0) || (map_z >= NUM_TRACK_CUBES))) {
        // off the map
...
static void CalculateRoadWheelHeight(long height, long* height_out) {
    ...
    long angle = (player_x_angle < (_180_DEGREES) ? (player_x_angle) : (player_x_angle - _360_DEGREES));
...
static void CalculateWorldRoadHeight(long wheel, long x, long z, long* y_out) {
...
#if defined(DEBUG) || defined(_DEBUG)
        if (debug_position)
            fprintf(out, "CalculateWorldRoadHeight error\n");
#endif
        if (first_time) {
...
#if defined(DEBUG) || defined(_DEBUG)
        if (debug_position)
            fprintf(out, "piece %d, this_piece %d\n", piece, this_piece);
#endif
        if ((first_time) || (abs(this_piece - piece) > 1)) // moved by more than one piece
        {
    ...
    while (before_surface) {
        ...
        xs = sx1 - sx4;
        zs = sz1 - sz4; // current segment vector
        xp = rx - sx4;
        zp = rz - sz4; // current point vector
    ...
    while (after_surface) {
        ...
        xs = sx3 - sx2;
        zs = sz3 - sz2; // current segment vector
        xp = rx - sx2;
        zp = rz - sz2; // current point vector
    ...
    if (wheel != CENTRE) // don't do this for CENTRE position (to allow road_x to include being off the road)
    {
        ...
        xs = sx2 - sx1;
        zs = sz2 - sz1; // current segment vector
        xp = rx - sx1;
        zp = rz - sz1; // current point vector
        off_left = (((xs * zp) - (xp * zs)) < 0 ? TRUE : FALSE);

        // calculate right dot product => off_right
        xs = sx4 - sx3;
        zs = sz4 - sz3; // current segment vector
        xp = rx - sx3;
        zp = rz - sz3; // current point vector
...
#if defined(DEBUG) || defined(_DEBUG)
    if (debug_position)
        fprintf(out, "GetSurfaceCoords sx2, sy2, sz2: 0x%x, 0x%x, 0x%x\n", sx2, sy2, sz2);
#endif
...
#if defined(DEBUG) || defined(_DEBUG)
    if (debug_position)
        fprintf(out, "GetSurfaceCoords sx3, sy3, sz3: 0x%x, 0x%x, 0x%x\n", sx3, sy3, sz3);
#endif
...
#if defined(DEBUG) || defined(_DEBUG)
    if (debug_position)
        fprintf(out, "GetSurfaceCoords sx1, sy1, sz1: 0x%x, 0x%x, 0x%x\n", sx1, sy1, sz1);
#endif
...
#if defined(DEBUG) || defined(_DEBUG)
    if (debug_position)
        fprintf(out, "GetSurfaceCoords sx4, sy4, sz4: 0x%x, 0x%x, 0x%x\n", sx4, sy4, sz4);
#endif
    return;
}
                                ...
                                long* segment_out) // only updated by the calculation for curves
{
    if (Track[piece].type & 0x80) // curve
    {
        ...
        if (piece_y_angle < 0)
            piece_y_angle = 0;
        if (piece_y_angle >= (MAX_ANGLE / 8))
            piece_y_angle = (MAX_ANGLE / 8) - 1;
        ...
        if (surface_x >= SURFACE_SIZE)
            surface_x = SURFACE_SIZE - 1;
        *sx = surface_x;
...
static void CalculateGravityAcceleration(void) {
    ...
    gravity_x_acceleration = ((-GRAVITY_ACCELERATION * static_cast<long>(trig_coeffs[X_Y_COMP])) >> LOG_PRECISION);

    // Acceleration along car's Y axis
    gravity_y_acceleration = ((-GRAVITY_ACCELERATION * static_cast<long>(trig_coeffs[Y_Y_COMP])) >> LOG_PRECISION);
...
static long front_difference_below_road, overall_difference_below_road;

static void CarCollisionDetection(void) {
    // local variables
    long difference;

    long average_front_amount_below_road, average_amount_below_road;
    ...
    if ((!touching_road) && (!on_chains)) {
        ...
        if (((angle < 0) && ((TrackID == ROLLER_COASTER) || (TrackID == SKI_JUMP))) || (angle >= 0)) {
            difference = -128;
...
static void CalculateCarCollisionAcceleration(long average_amount_below_road) {
    ...
    front_height_difference = (front_left_height_difference + front_right_height_difference) >> 1;
    x_inclination_to_road = (front_height_difference - rear_height_difference) >> log_car_length_factor;

    // Calculate sin and cos of X angle between car and road surface
    CalculateInclinationSinCos(x_inclination_to_road, &surface_sinx, &surface_cosx);

    // Calculate z_inclination_to_road
    z_inclination_to_road = (front_left_height_difference - front_right_height_difference) >> log_car_width_factor;

    // Calculate sin and cos of Z angle between car and road surface
    CalculateInclinationSinCos(z_inclination_to_road, &surface_sinz, &surface_cosz);
...
static long Cosine_Conversion_Table[] =
    ...
    {};
...
static void CalculateTotalAcceleration(void) {
    ...
    player_z_acceleration = engine_z_acceleration + gravity_z_acceleration + car_collision_z_acceleration;
...
static void CalculateSteering(void) {
    ...
    if (y_angle_difference > (_180_DEGREES))
        y_angle_difference -= (_360_DEGREES);
    if (y_angle_difference < -(_180_DEGREES))
        y_angle_difference += (_360_DEGREES);
    // this value should go increasingly -'ve when turning to the right
    // and should go increasingly +'ve when turning to the left
...
static void IdentifyPiece(long x, long z, long* piece_in_out) {
    ...
    while (before_piece) {
        ...
        xp = rx - px4;
        zp = rz - pz4; // current point vector
    ...
    while (after_piece) {
        ...
        xp = rx - px2;
        zp = rz - pz2; // current point vector
...
static void UpdatePlayersPosition(void) {
    ...
    // get angle in Amiga StuntCarRacer format (i.e. correct sign)
    angle = (player_x_angle < (_180_DEGREES) ? (player_x_angle) : (player_x_angle - _360_DEGREES));
    ...
    if (abs(angle) > limit) {
        ...
        if (((player_x_rotation_speed >= 0) && (angle < 0)) || ((player_x_rotation_speed < 0) && (angle >= 0))) {
    ...
    // get angle in Amiga StuntCarRacer format (i.e. correct sign)
    angle = (player_z_angle < (_180_DEGREES) ? (player_z_angle) : (player_z_angle - _360_DEGREES));
    ...
    if (abs(angle) > limit) {
        ...
        if (((player_z_rotation_speed >= 0) && (angle < 0)) || ((player_z_rotation_speed < 0) && (angle >= 0))) {
            // values have different signs
            player_z_rotation_speed = 0;
        }
    }
...
long CalculateDisplaySpeed(void) {
    ...
    if (speed < 0x1100)
        speed = 0; // first few values are not displayed
...
static void DrawSparks(void) {
    ...
    if (NOT_WRECKED)
        return; // if car is not scraping on road

on_an_edge:
    if (off_map_status != 0)
        return; // dust clouds will be drawn instead

    p = abs(player_z_speed) >> 8;
    if (p < 1)
        return; // if speed is not large enough
...
void UpdateLapData(void) {
    ...
    long previousLap[NUM_CARS];
    previousLap[OPPONENT] = lapNumber[OPPONENT];
    previousLap[PLAYER] = lapNumber[PLAYER];
    ...
    for (car = OPPONENT; car < NUM_CARS; car++) {
        if (lapNumber[car] > previousLap[car])
            OnLapNumberIncreased(car, previousLap[car]);
    }
...
bool FormatCurrentLapTimeForHud(wchar_t* out, size_t outChars) {
    if (!out || outChars == 0)
        return false;
    out[0] = L'\0';
...
bool FormatBestLapTimeForHud(wchar_t* out, size_t outChars) {
    if (!out || outChars == 0)
        return false;
    out[0] = L'\0';
    if (!g_lapBestValid)
...
bool IsBestLapTimePlayers(void) {
    return g_lapBestValid && g_lapBestIsPlayer;
}
...
bool GetRecordedAmigaWord(long* value_out) {
    ...
    if (!ReplayAmigaRecording)
        return (FALSE);

    if (!OpenAmigaRecording())
        return (FALSE);
    ...
    if (i == 0) {
        ...
        if (e)
            fprintf(out, "Can't read Amiga word correctly (%d)\n", e);
...
bool GetRecordedAmigaLong(long* value_out) {
    ...
    if (i == 0) {
        int e = ferror(AmigaFile);
        if (e)
            fprintf(out, "Can't read Amiga long correctly (%d)\n", e);
...
void CompareRecordedAmigaWord(char* name, long* value) {
    ...
    if (!GetRecordedAmigaWord(&amiga_value))
        return;
